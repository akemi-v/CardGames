...
-(void)updateCardButton:(UIButton *)cardButton  usingCard:(Card *)card
{
    if ([card isKindOfClass:[SetCard class]]) {
        ...
        cardButton.alpha = card.isMatched ? 0.2 : 1.0;
    }
}
...
- (NSAttributedString *)cardAttributedContents:(SetCard *)card
{
    ...
    NSString *textToDisplay =  [@"" stringByPaddingToLength:card.number withString:symbolPallette[card.symbol] startingAtIndex:0];
...
{
    NSMutableAttributedString *text = [[NSMutableAttributedString alloc] initWithAttributedString:[self cardAttributedContents:(SetCard *)card]];
    [text appendAttributedString:[[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@",(card.isChosen) ? @" selected!" : @" de-selected!"]]];
    return text;
}
